...
void LedDriver_Init(I2C_HandleTypeDef *hi2c){
    ...
    result = LedDriver_WriteReg(SHUTDOWN_REG_ADDR, NORMAL_OPERATION_MODE, hi2c);
    result = LedDriver_WriteReg(SCAN_LIMIT_REG_ADDR, DISPLAY_7_DIGITS, hi2c);    
    result = LedDriver_WriteReg(GLOBAL_INTENSITY_REG_ADDR, MID_INTENSITY, hi2c);
  }
